...
const NavLink: React.FC<NavLinkProps> = ({ item, onClick }) => {
  ...
  const linkclasses = clsx(
    "py-2 sm:py-3 text-xl xs:text-xl sm:text-2xl md:text-3xl lg:text35xl font-medium text-white/40 rounded-full group-hover:text-primary transition-all duration-300",
    {
  ...
  const liststyle = clsx("w-0 h-0.5 bg-primary transition-all duration-300", {
    ...
    "group-hover:block group-hover:w-4 sm:group-hover:w-6 group-hover:mr-2 sm:group-hover:mr-4":
      true,
  });
